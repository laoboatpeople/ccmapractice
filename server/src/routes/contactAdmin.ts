 ...
 * Admin messages (from info@ccmapractice.com) are merged into the
...
router.get('/conversations', authenticate, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const ADMIN_EMAILS = ['info@ccmapractice.com'];
  ...
  try {
    ...
    if (apiKey) {
      const emailText = [
        ...
        '',
        '---',
        'ccmapractice — ICC Building Inspector Exam Preparation',
        'https://ccmapractice.com',
      ].join('\n');
      ...
      try {
        await fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({
            from: FROM_EMAIL,
            to: [original.email],
            reply_to: 'info@ccmapractice.com',
            subject: `Re: [ccmapractice] Contact from ${original.name}`,
            text: emailText,
          }),
        });
      } catch (emailErr) {
