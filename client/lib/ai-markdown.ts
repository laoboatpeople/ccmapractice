...
export function renderAIResponse(content: string): string {
  ...
  html = html.replace(/\\\[([\s\S]*?)\\\]/g, (_m, math: string) => {
    ...
    return `<pre class="bg-[#150A12] border border-border p-3 rounded-lg my-2.5 text-sm font-mono overflow-x-auto text-text-primary">${cleaned}</pre>`;
  });

  // Inline math \( ... \)
  html = html.replace(/\\\(([\s\S]*?)\\\)/g, (_m, math: string) => {
    const cleaned = cleanLatex(math);
    return `<code class="bg-[#150A12] px-2 py-0.5 rounded text-xs font-mono text-text-primary">${cleaned}</code>`;
  });
  ...
  html = html.replace(/((?:^\s*\|.*\|\s*$[\r\n]*)+)/gm, (block) => {
    ...
    if (header) {
      t += '<thead><tr>' + cells(header)
        .map((c) => `<th class="border border-border bg-[#150A12] px-2 py-1.5 text-left font-semibold text-text-primary">${c}</th>`)
        .join('') + '</tr></thead>';
    }
    t += '<tbody>';
    for (const r of body) {
