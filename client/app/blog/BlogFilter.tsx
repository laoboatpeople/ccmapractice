      ...
      <div className="max-w-4xl mx-auto px-6 pb-24 space-y-6">
        {filteredPosts.map((post) => (
          <Link
            ...
            className="block p-6 rounded-xl border border-white/10 bg-white/[0.02] hover:bg-white/[0.05] hover:border-[#DB2777]/30 transition-all group"
          >
            <div className="flex items-center gap-3 mb-3">
              <span className="text-[11px] font-medium text-[#DB2777] bg-[#DB2777]/10 px-2 py-1 rounded">
                {post.category}
              </span>
              <span className="text-xs text-[#64748B]">{post.date}</span>
              <span className="text-xs text-[#64748B]">· {post.readTime}</span>
            </div>
            <h2 className="text-xl font-semibold mb-2 group-hover:text-[#DB2777] transition-colors">
              {post.title}
            </h2>
